...
def map_to_game_summary(all_penalty_records_of_player, participant_total_fine):
    ...
    final_penalty_record = PenaltyRecord(all_penalty_records_of_player[0].game_id,list_of_final_penalties,all_penalty_records_of_player[0].participant.username,all_penalty_records_of_player[0].participant.id, participant_total_fine)
    return final_penalty_record

def map_penalty_record_for_game_summary(penalty_record):
    penalty_summary = Penalty(penalty_record.id,penalty_record.penalty.title,penalty_record.penalty.pay_amount,penalty_record.quantity)
